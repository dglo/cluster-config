...
snBuilder-?-service.xml files.  This stylesheet is also used for
inclusion in the cluster-services.xsl file to pick up the template for
modules named "snBuilder". -->
  ...
  <xsl:template match="/">
    <xsl:apply-templates select="icecube/cluster[@name=$cluster-name]/location/module[name='snBuilder']" />
  </xsl:template>

  <!-- Match snBuilder modules, for use as an imported stylesheet -->
  <xsl:template match="module[name='snBuilder']">
    <!-- xsl:message>Generating for <xsl:value-of select="$cluster-name" />: <xsl:value-of select="concat($build-dir, '/snBuilder-', id, '-service.xml')" /></xsl:message -->
    <redirect:write select="concat($build-dir, '/snBuilder_', id, '-service.xml')">
      <server>
        ...
        <mbean code="icecube.daq.snBuilder.SNBuilderManager"
               name="iceCube.daq:type=snBuilder,id={id},acme-aspect=control,name=snBuilderCommands"
               xmbean-dd="XMBEAN-INF/icecube.daq.snBuilder.SNBuilderManager.xml">
          <attribute name="DAQCompID"><xsl:value-of select="id"/></attribute>
          <attribute name="DAQCompName">snBuilder</attribute>
          ...
          <depends optional-attribute-name="DaqDispatchJNDIService">IceCubeDAQ.dispatch:acme-aspect=configuration,component=dd,dd=dispatch,dispatch=sn,type=devnull</depends>
          ...
          <xsl:if test="$cluster-name='merc'">
            <depends-list>
              ...
              <depends-list-element>jboss.j2ee:module=snBuilder-prod-conf-ejb.jar,service=EjbModule</depends-list-element>
            </depends-list>
          </xsl:if>
        </mbean>
      </server>
