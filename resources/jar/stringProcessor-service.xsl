...
stringProcessor-?-service.xml files.  This stylesheet is also used for
inclusion in the cluster-services.xsl file to pick up the template for
modules named "stringProcessor". -->
  ...
  <xsl:template match="/">
    <xsl:apply-templates select="icecube/cluster[@name=$cluster-name]/location/module[name='stringProcessor']" />
  </xsl:template>

  <!-- Match stringProc modules, for use as an imported stylesheet -->
  <xsl:template match="module[name='stringProcessor']">
    <!-- xsl:message>Generating for <xsl:value-of select="$cluster-name" />: <xsl:value-of select="concat($build-dir, '/stringProcessor-', id, '-service.xml')" /></xsl:message -->
    <redirect:write select="concat($build-dir, '/stringProcessor_', id, '-service.xml')">
      <server>
        ...
        <mbean code="icecube.daq.stringProcessor.StringProcessorManager"
               name="iceCube.daq:type=stringProcessor,id={id},acme-aspect=control,name=stringProcessorCommands"
               xmbean-dd="XMBEAN-INF/icecube.daq.stringProcessor.StringProcessorManager.xml">
          <attribute name="DAQCompID"><xsl:value-of select="id"/></attribute>
          <attribute name="DAQCompName">stringProcessor</attribute>
