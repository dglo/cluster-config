...
tcalBuilder-?-service.xml files.  This stylesheet is also used for
inclusion in the cluster-services.xsl file to pick up the template for
modules named "tcalBuilder". -->
  ...
  <xsl:template match="/">
    <xsl:apply-templates select="icecube/cluster[@name=$cluster-name]/location/module[name='tcalBuilder']" />
  </xsl:template>

  <!-- Match tcalBuilder modules, for use as an imported stylesheet -->
  <xsl:template match="module[name='tcalBuilder']">
    <!-- xsl:message>Generating for <xsl:value-of select="$cluster-name" />: <xsl:value-of select="concat($build-dir, '/tcalBuilder-', id, '-service.xml')" /></xsl:message -->
    <redirect:write select="concat($build-dir, '/tcalBuilder_', id, '-service.xml')">
      <server>
        ...
        <mbean code="icecube.daq.tcalBuilder.TcalBuilderManager"
               name="iceCube.daq:type=tcalBuilder,id={id},acme-aspect=control,name=tcalBuilderCommands"
               xmbean-dd="XMBEAN-INF/icecube.daq.tcalBuilder.TcalBuilderManager.xml">
          <attribute name="DAQCompID"><xsl:value-of select="id"/></attribute>
          <attribute name="DAQCompName">tcalBuilder</attribute>
          ...
          <depends optional-attribute-name="DaqDispatchJNDIService">IceCubeDAQ.dispatch:acme-aspect=configuration,component=dd,dd=dispatch,dispatch=tcal,type=devnull</depends>
