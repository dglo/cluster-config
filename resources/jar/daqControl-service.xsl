...
daqControl-?-service.xml files.  This stylesheet is also used for
inclusion in the cluster-services.xsl file to pick up the template for
modules named "daqControl". -->
  ...
  <xsl:template match="/">
    <xsl:apply-templates select="icecube/cluster[@name=$cluster-name]/location/module[name='daqControl']" />
  </xsl:template>

  <!-- Match daqControl modules, for use as an imported stylesheet -->
  <xsl:template match="module[name='daqControl']">
    <!-- xsl:message>Generating for <xsl:value-of select="$cluster-name" />: <xsl:value-of select="concat($build-dir, '/daqControl-', id, '-service.xml')" /></xsl:message -->
    <redirect:write select="concat($build-dir, '/daqControl_', id, '-service.xml')">
      <server>
        ...
        <mbean code="icecube.daq.daqControl.DAQcontrolManager"
               name="iceCube.daq:type=daqControl,id={id},acme-aspect=control,name=daqControlCommands"
               xmbean-dd="XMBEAN-INF/icecube.daq.daqControl.DAQcontrolManager.xml">
          <attribute name="DAQCompID"><xsl:value-of select="id"/></attribute>
          <attribute name="DAQCompName">daqControl</attribute>
