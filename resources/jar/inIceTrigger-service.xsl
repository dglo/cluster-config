...
iniceTrigger-?-service.xml files.  This stylesheet is also used for
inclusion in the cluster-services.xsl file to pick up the template for
modules named "inIceTrigger". -->
  ...
  <xsl:template match="/">
    <xsl:apply-templates select="icecube/cluster[@name=$cluster-name]/location/module[name='inIceTrigger']" />
  </xsl:template>

  <!-- Match inIceTrigger modules, for use as an imported stylesheet -->
  <xsl:template match="module[name='inIceTrigger']">
    <!-- xsl:message>Generating for <xsl:value-of select="$cluster-name" />: <xsl:value-of select="concat($build-dir, '/inIceTrigger-', id, '-service.xml')" /></xsl:message -->
    <redirect:write select="concat($build-dir, '/inIceTrigger_', id, '-service.xml')">
      <server>
        ...
        <mbean code="icecube.daq.iniceTrigger.IniceTriggerManager"
               name="iceCube.daq:type=inIceTrigger,id={id},acme-aspect=control,name=inIceTriggerCommands"
               xmbean-dd="XMBEAN-INF/icecube.daq.iniceTrigger.IniceTriggerManager.xml">
          <attribute name="DAQCompID"><xsl:value-of select="id"/></attribute>
          <attribute name="DAQCompName">inIceTrigger</attribute>
          ...
          <xsl:if test="$cluster-name='merc'">
            <depends-list>
              ...
              <depends-list-element>jboss.j2ee:module=triggerConfig-ejb.jar,service=EjbModule</depends-list-element>
            </depends-list>
          </xsl:if>
        </mbean>
      </server>
